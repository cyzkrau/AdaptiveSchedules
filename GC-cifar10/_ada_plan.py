...
def ada_del_plan(points, target_length, end_step, smb, L, C):
    plan = list(range(-1,end_step))
    smb.insert(0, 0)
    while len(plan)-1 > target_length:
        error = []
        for m in range(1,len(plan)-1):
            tk1, tk2, tk3 = plan[m-1], plan[m], plan[m+1]
            se = smb[tk3] * (points[tk3]-points[tk2]) + smb[tk2] * (points[tk2]-points[tk1])
            de = L(points[tk3]) * (points[tk3]-points[tk2])**2 + L(points[tk2]) * (points[tk2]-points[tk1])**2
            error.append(se+C*de)
            tk1, tk2, tk3 = plan[m-1], plan[m-1], plan[m+1]
            se = smb[tk3] * (points[tk3]-points[tk2]) + smb[tk2] * (points[tk2]-points[tk1])
            de = L(points[tk3]) * (points[tk3]-points[tk2])**2 + L(points[tk2]) * (points[tk2]-points[tk1])**2
            error[-1] -= se+C*de
        m = np.argmax(error)
