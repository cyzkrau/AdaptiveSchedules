...
for steps in [5, 10, 20, 50, 100]:
    for num_iter in range(6):
        def adjust(_plan):
            plan = _plan.numpy()
            min_last = plan[0] - plan[1]
            plan = list(plan[::-1])
        ...
        while _nums > 0:
            ...
            for i in image:
                i.save(f"./image/aadm-lip/aadm-{steps:04d}steps-{num_iter:02d}iter/{_nums:05d}.png")
                _nums -= 1
        ...
        dataset = ImageDataset(f"./image/aadm-lip/aadm-{steps:04d}steps-{num_iter:02d}iter/", nums, transform)
        fid_result = fid.calculate_fid(DataLoader(dataset, batch_size=512, shuffle=False))
        with open("./log50k.txt", 'a') as f:
            f.write(f"lip-{steps:04d}-{num_iter:02d}: {fid_result:.4f}\n")
